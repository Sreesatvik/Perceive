...
- **On-Device Multimodal Perception**: Face detection (`@mediapipe/tasks-vision`) and OCR (`tesseract.js`) run entirely in-browser, feeding the same PII-tokenization pipeline as DOM-detected text — raw pixels never leave the browser. OCR is accuracy-measured against a real fixture set (100% recall/precision on PII-pattern matches, incl. the canvas-rendered "zero DOM cooperation" case — see `docs/vision-accuracy/ocr-results.json`). Face detection is measured for real in live Chrome at 39.1% recall / 60% precision overall — **reliable on single large faces (67-100% recall: frontal, angled, occluded, ID-card-photo scenarios)**, **unreliable on small/distant faces (0%) and crowded multi-face frames (20%)** — see `docs/vision-engine-decision.md` for the full breakdown and scope decision.
