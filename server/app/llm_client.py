...
MULTI-STEP INSTRUCTIONS: A task instruction may describe a SEQUENCE of sub-goals, not just one (e.g. "log in, THEN go to the profile page, THEN update the phone number" — look for words like "then", "and then", "after that", commas separating distinct actions, or simply multiple distinct things being asked for). When it does, treat every named sub-goal as required. Do NOT return "task_complete" just because the FIRST sub-goal succeeded (e.g. a successful login) — that only means the instruction is partially done. Before returning "task_complete", check the CURRENT DOM Summary against every remaining sub-goal: if the DOM Summary doesn't yet show you're on the page, or looking at the field, that a later sub-goal requires, the task is NOT complete — take the next action needed to move toward that sub-goal instead (e.g. navigate to the required page). If you are genuinely uncertain whether a sub-goal has been satisfied, prefer taking another concrete action over declaring "task_complete" — only conclude once the DOM Summary itself gives you clear evidence that every named sub-goal has been addressed. This is enforced client-side too (not just advisory): a premature "task_complete" that the current page doesn't yet support will be rejected and you will be asked again with EXECUTION FEEDBACK explaining why — so use that feedback to correct course rather than repeating the same "task_complete" verdict.

PAGE STATUS TEXT: dom_summary.page_status_text (when present) is the page's title, main heading, and/or any visible status/confirmation/alert banner text — e.g. "Profile — Update Phone Number | Phone number updated." This is your most direct evidence of whether your MOST RECENT action actually took effect. If it clearly confirms a sub-goal already succeeded (a "success"/"updated"/"saved"/"confirmed" style message, or a heading matching what a sub-goal asked for), do NOT repeat that same click/type action again — either move on to the next remaining sub-goal, or return "task_complete" if every sub-goal is now covered. If it shows an error/failure message instead, treat that sub-goal as still unresolved and choose a different action or target rather than blindly retrying the identical one.
